export { IEntity } from './IEntity';
export { IPage, IPaging, defaultPaging } from './IPage';
export { IRead } from './IRead';
export { IRepository, IRepositoryWithCache } from './IRepository';
export { IRepositoryFactory } from './IRepositoryFactory';
export { IUnitOfWork, IUnitOfWorkOptions } from './IUnitOfWork';
export { IWrite } from './IWrite';
export { ICache } from './ICache';
export { IAuditable } from './IAuditable';
export { IProtectedResource } from './IProtectedResource';
export { ResourceAccess } from './IProtectedResource';
export { Access } from './IProtectedResource';